...
<link rel="alternate" type="application/rss+xml" title="{BRAND} 과천 안내 RSS" href="{BASE_URL.rstrip('/')}/rss.xml">
  ...
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <div class="footer-cta">
        <a class="footer-cta-btn" href="{TELEGRAM_BUILD}" target="_blank" rel="noopener nofollow">
    ...
    <nav class="footer-col" aria-label="정책 및 기준">
      ...
      <ul>
        ...
        <li><a href="/reservation/#move">추가 이동비 안내</a></li>
      </ul>
    </nav>
  </div>
...
def build() -> None:
    ...
    for page in PAGES:
        ...
        if desc_len > MAX_DESC_CHARS:
            long_descs.append((path or "/", desc_len))

        chars = text_length(page["body"])
        noindex = page.get("noindex", False) or chars < MIN_INDEX_CHARS
        if not noindex:
            sitemap_items.append((base + "/" + path, page["title"], page.get("desc", "")))
        report.append((path or "/", chars, "noindex" if noindex else "index"))

    # sitemap.xml (lastmod 포함 — 크롤러 신선도 신호)
    urls = "\n".join(
        f"  <url><loc>{u}</loc><lastmod>{today}</lastmod>"
        f"<changefreq>weekly</changefreq></url>"
        for u, _, _ in sitemap_items
    )
    ...
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            f"  <title>{html.escape(BRAND)} · 과천 출장마사지·홈타이 안내</title>\n"
            f"  <link>{base}/</link>\n"
            f"  <description>과천시 방문형 관리(출장마사지·홈타이) 지역·역세권·생활권 안내</description>\n"
            "  <language>ko</language>\n"
            f"  <lastBuildDate>{now_rfc822}</lastBuildDate>\n"
            f"  <atom:link xmlns:atom=\"http://www.w3.org/2005/Atom\" href=\"{base}/rss.xml\" rel=\"self\" type=\"application/rss+xml\"/>\n"
            f"{items}\n  </channel></rss>\n"
        )
    ...
    if long_descs:
        print(f"\n⚠ 디스크립션 {MAX_DESC_CHARS}자 초과 페이지:")
        for p, n in long_descs:
            print(f"  {p}  ({n}자)")
    else:
        print(f"모든 디스크립션 {MAX_DESC_CHARS}자 이내 ✓")
